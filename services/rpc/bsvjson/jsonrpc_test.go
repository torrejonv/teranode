...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if bsvjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *bsvjson.RPCError {
				return bsvjson.NewRPCError(bsvjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		...
		marshalled, err := bsvjson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := bsvjson.NewRequest(nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := bsvjson.Error{ErrorCode: bsvjson.ErrInvalidType}

	_, err = bsvjson.MarshalResponse(make(chan int), nil, nil)
	if jerr, ok := err.(bsvjson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = bsvjson.MarshalResponse(1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			bsvjson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			bsvjson.ErrRPCMethodNotFound,
