...
func ExampleMarshalCmd() {
	...
	marshalledBytes, err := bsvjson.MarshalCmd(id, gbCmd)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleUnmarshalCmd() {
	...
	gbCmd, ok := cmd.(*bsvjson.GetBlockCmd)
	if !ok {
...
func ExampleMarshalResponse() {
	...
	marshalledBytes, err := bsvjson.MarshalResponse(1, 350001, nil)
	if err != nil {
		fmt.Println(err)
		return
	}
