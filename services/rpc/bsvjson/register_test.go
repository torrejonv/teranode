...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		in   bsvjson.UsageFlag
		want string
	}{
		{0, "0x0"},
		{bsvjson.UFWalletOnly, "UFWalletOnly"},
		{bsvjson.UFWebsocketOnly, "UFWebsocketOnly"},
		{bsvjson.UFNotification, "UFNotification"},
		{bsvjson.UFWalletOnly | bsvjson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{bsvjson.UFWalletOnly | bsvjson.UFWebsocketOnly | (1 << 31),
	...
	highestUsageFlagBit := bsvjson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			err: bsvjson.Error{ErrorCode: bsvjson.ErrDuplicateMethod},
		},
		{
			...
			flags: bsvjson.TstHighestUsageFlagBit,
			err:   bsvjson.Error{ErrorCode: bsvjson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: bsvjson.Error{ErrorCode: bsvjson.ErrEmbeddedType},
		},
		{
			...
			err: bsvjson.Error{ErrorCode: bsvjson.ErrUnexportedField},
		},
		{
			...
			err: bsvjson.Error{ErrorCode: bsvjson.ErrUnsupportedFieldType},
		},
		{
		...
		},
		{
			...
			err: bsvjson.Error{ErrorCode: bsvjson.ErrNonOptionalField},
		},
		{
			...
			err: bsvjson.Error{ErrorCode: bsvjson.ErrNonOptionalDefault},
		},
		{
			...
			err: bsvjson.Error{ErrorCode: bsvjson.ErrMismatchedDefault},
		},
	}
	...
	for i, test := range tests {
		err := bsvjson.RegisterCmd(test.method, test.cmdFunc(),
			test.flags)
...
func TestMustRegisterCmdPanic(t *testing.T) {
	...
	bsvjson.MustRegisterCmd("panicme", 0, 0)
}
