...
func TestHelpReflectInternals(t *testing.T) {
	...
	for i, test := range tests {
		...
		key := bsvjson.TstReflectTypeToJSONType(xT, test.reflectType)
		if key != test.key {
		...
		examples, isComplex := bsvjson.TstReflectTypeToJSONExample(xT,
		...
		helpText := bsvjson.TstResultTypeHelp(xT, test.reflectType, "fdk")
		...
		isValid := bsvjson.TstIsValidResultType(test.reflectType.Kind())
...
func TestResultStructHelp(t *testing.T) {
	...
	for i, test := range tests {
		results := bsvjson.TstResultStructHelp(xT, test.reflectType, 0)
...
func TestHelpArgInternals(t *testing.T) {
	...
	for i, test := range tests {
		help := bsvjson.TstArgHelp(xT, test.reflectType, test.defaults,
			test.method)
		if help != test.help {
...
func TestMethodHelp(t *testing.T) {
	...
	for i, test := range tests {
		help := bsvjson.TestMethodHelp(xT, test.reflectType,
...
func TestGenerateHelpErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			method: "boguscommand",
			err:    bsvjson.Error{ErrorCode: bsvjson.ErrUnregisteredMethod},
		},
		{
		...
		},
		{
			...
			err:         bsvjson.Error{ErrorCode: bsvjson.ErrMissingDescription},
		},
	}
	...
	for i, test := range tests {
		_, err := bsvjson.GenerateHelp(test.method, nil,
			test.resultTypes...)
...
func TestGenerateHelp(t *testing.T) {
	...
	help, err := bsvjson.GenerateHelp("help", descs)
	if err != nil {
