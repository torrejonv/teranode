...
func (c *validatorAPIClient) Subscribe(ctx context.Context, in *SubscribeRequest, opts ...grpc.CallOption) (ValidatorAPI_SubscribeClient, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ValidatorAPI_ServiceDesc.Streams[1], ValidatorAPI_Subscribe_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &validatorAPISubscribeClient{ClientStream: stream}
...
func _ValidatorAPI_Subscribe_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ValidatorAPIServer).Subscribe(m, &validatorAPISubscribeServer{ServerStream: stream})
}
