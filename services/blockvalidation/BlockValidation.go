...
func (u *BlockValidation) _(ctx context.Context, blockHash *chainhash.Hash) error {
	...
	/*
		...
		oldBlockIDsMap.Range(func(txID, blockIDs interface{}) bool {
			...
			if err != nil {
				u.logger.Errorf("[BlockValidation][%s] failed to check if old blocks are part of the current chain: %v", block.String(), err)
			}
	...
	*/
	...
	return util.IterateOnSyncMap(ctx, oldBlockIDsMap, u.blockchainClient, u.logger, block.String())
}
...
func (u *BlockValidation) ValidateBlock(ctx context.Context, block *model.Block, baseURL string, bloomStats *model.BloomStats, disableOptimisticMining ...bool) error {
	...
	if useOptimisticMining {
		...
		*/
		...
		iterationError := util.IterateOnSyncMap(ctx, oldBlockIDsMap, u.blockchainClient, u.logger, block.String())
...
func (u *BlockValidation) reValidateBlock(blockData revalidateBlockData) error {
	...
	/*
		...
		oldBlockIDsMap.Range(func(txID, blockIDs interface{}) bool {
			...
			// if err is not nil, log the error and continue iterating for the next transaction
			if err != nil {
				u.logger.Errorf("[ValidateBlock][%s] failed to check if old blocks are part of the current chain: %v", blockData.block.String(), err)
			}
	...
	*/
	...
	return util.IterateOnSyncMap(ctx, oldBlockIDsMap, u.blockchainClient, u.logger, blockData.block.String())
}
