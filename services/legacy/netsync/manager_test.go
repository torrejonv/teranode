...
func TestMempoolSync(t *testing.T) {
	...
	blockVersion := int32(2)
	timestamp := time.Now().Truncate(time.Second)
	block, err := CreateBlock(genesisBlock, nil, blockVersion,
		timestamp, address, []wire.TxOut{}, &chainParams)
	if err != nil {
		t.Fatalf("failed to generate block: %v", err)
	}
	...
	case call := <-ctx.peerNotifier.announceNewTransactionsChan:
		if len(call.newTxs) != 1 || !call.newTxs[0].IsEqual(tx1.Hash()) {
			t.Fatalf("PeerNotifier received unexpected AnnounceNewTransactions call: %v", call.newTxs)
		}
	default:
		t.Fatal("Expected SyncManager to make AnnounceNewTransactions call " +
			"to PeerNotifier")
	...
	case call := <-ctx.peerNotifier.announceNewTransactionsChan:
		if len(call.newTxs) != 2 ||
			!call.newTxs[0].IsEqual(tx2.Hash()) || !call.newTxs[1].IsEqual(tx3.Hash()) {
			t.Fatalf("PeerNotifier received unexpected AnnounceNewTransactions call: %v", call.newTxs)
		}
	default:
		t.Fatal("Expected SyncManager to make AnnounceNewTransactions call " +
