...
func ChainIntegrity(checkInterval int, alertThreshold int, debug bool, logfile string) {
	...
	tSettings := settings.NewSettings("docker.host.teranode1")
	...
	for i := len(blockHeaders) - 1; i >= 0; i-- {
		logger.Debugf("blockHeaders: accessing index %d (len=%d)", i, len(blockHeaders))
		...
		if block.CoinbaseTx.Inputs[0].UnlockingScript.String() != genesisScript {
			...
			for vout, output := range block.CoinbaseTx.Outputs {
				logger.Debugf("block.CoinbaseTx.Outputs: accessing index %d (len=%d)", vout, len(block.CoinbaseTx.Outputs))
			...
			for _, subtreeHash := range block.Subtrees {
				...
				for nodeIdx, node := range subtree.Nodes {
					...
					logger.Debugf("subtree.Nodes: accessing index %d (len=%d)", nodeIdx, len(subtree.Nodes))
					...
					if !util.CoinbasePlaceholderHash.Equal(node.Hash) {
						...
						for vout, output := range btTx.Outputs {
							logger.Debugf("btTx.Outputs: accessing index %d (len=%d)", vout, len(btTx.Outputs))
