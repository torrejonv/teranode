	...
	for h := 0; h <= height; h++ {
		...
		if merkleTo-merkleFrom > routineSplitSize {
			...
			for i := merkleFrom; i < merkleTo; i += routineSplitSize {
				...
				go func(i int) {
					defer wg.Done()
					st.calcMerkles(i, Min(i+routineSplitSize, merkleTo), nextPoT, length, merkles)
				}(i)
			}
			wg.Wait()
		} else {
			st.calcMerkles(merkleFrom, merkleTo, nextPoT, length, merkles)
		}
...
func (st *Subtree) calcMerkles(merkleFrom, merkleTo, nextPoT, length int, merkles []chainhash.Hash) {
	var offset int
	...
	for i := merkleFrom; i < merkleTo; i += 2 {
		...
		merkles[offset] = st.calcMerkle(currentMerkle, currentMerkle1)
	}
}

func (st *Subtree) calcMerkle(currentMerkle chainhash.Hash, currentMerkle1 chainhash.Hash) [32]byte {
	switch {
