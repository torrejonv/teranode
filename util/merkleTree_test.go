...
func TestGetMerkleProofForCoinbase(t *testing.T) {
	...
	t.Run("", func(t *testing.T) {
		subtree1 := NewTree(2)
		require.NoError(t, subtree1.AddNode(hash1, 12))
		require.NoError(t, subtree1.AddNode(hash2, 13))
		require.NoError(t, subtree1.AddNode(hash3, 14))
		require.NoError(t, subtree1.AddNode(hash4, 15))

		subtree2 := NewTree(2)
		require.NoError(t, subtree2.AddNode(hash5, 16))
		require.NoError(t, subtree2.AddNode(hash6, 17))
		require.NoError(t, subtree2.AddNode(hash7, 18))
		require.NoError(t, subtree2.AddNode(hash8, 19))
		...
		require.NoError(t, topTree.AddNode(subtree1.RootHash(), subtree1.Fees))
		require.NoError(t, topTree.AddNode(subtree2.RootHash(), subtree2.Fees))
