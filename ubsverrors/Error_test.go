...
func Test_NewCustomError(t *testing.T) {
	err := New(ErrorConstants_NOT_FOUND, "resource not found")
	if err == nil {
		t.Fatalf("expected non-nil error")
	}

	if err.Code != ErrorConstants_NOT_FOUND {
		t.Errorf("expected code %v; got %v", ErrorConstants_NOT_FOUND, err.Code)
	}
...
func Test_WrapGRPC(t *testing.T) {
	originalErr := New(ErrorConstants_NOT_FOUND, "not found")
...
func Test_UnwrapGRPC(t *testing.T) {
	...
	if !errors.Is(unwrappedErr, New(ErrorConstants_NOT_FOUND, "")) {
...
func Test_ErrorIs(t *testing.T) {
	err := New(ErrorConstants_NOT_FOUND, "not found")
	if !errors.Is(err, New(ErrorConstants_NOT_FOUND, "")) {
		t.Errorf("errors.Is failed to recognize NOT_FOUND error type")
	}

	err = New(ErrorConstants_INVALID_BLOCK, "invalid block error")
	if !errors.Is(err, New(ErrorConstants_INVALID_BLOCK, "")) {
		t.Errorf("errors.Is failed to recognize INVALID_BLOCK error type")
	}

	err = New(ErrorConstants_INVALID_TX_DOUBLE_SPEND, "invalid tx double spend error")
	if !errors.Is(err, New(ErrorConstants_INVALID_TX_DOUBLE_SPEND, "")) {
		t.Errorf("errors.Is failed to recognize INVALID_TX_DOUBLE_SPEND error type")
	}

	err = New(ErrorConstants_THRESHOLD_EXCEEDED, "threshold exceeded error")
	if !errors.Is(err, New(ErrorConstants_THRESHOLD_EXCEEDED, "")) {
		t.Errorf("errors.Is failed to recognize THRESHOLD_EXCEEDED error type")
	}

	err = New(ErrorConstants_BLOCK_NOT_FOUND, "block not found error")
	if !errors.Is(err, New(ErrorConstants_BLOCK_NOT_FOUND, "")) {
		t.Errorf("errors.Is failed to recognize BLOCK_NOT_FOUND error type")
	}

	err = New(ErrorConstants_UNKNOWN, "unknown error")
	if !errors.Is(err, New(ErrorConstants_UNKNOWN, "")) {
		t.Errorf("errors.Is failed to recognize UNKNOWN error type")
	}

	err = New(ErrorConstants_INVALID_ARGUMENT, "invalid argument error")
	if !errors.Is(err, New(ErrorConstants_INVALID_ARGUMENT, "")) {
		t.Errorf("errors.Is failed to recognize INVALID_ARGUMENT error type")
	}
}

func Test_ErrorWrapWithAdditionalContext(t *testing.T) {
	originalErr := New(ErrorConstants_INVALID_TX_DOUBLE_SPEND, "original error")
...
func Test_ErrorEquality(t *testing.T) {
	err1 := New(ErrorConstants_NOT_FOUND, "resource not found")
	err2 := New(ErrorConstants_NOT_FOUND, "resource not found")
	...
	// same error codes
	err2 = New(ErrorConstants_NOT_FOUND, "invalid argument")
	...
	err2 = New(ErrorConstants_INVALID_ARGUMENT, "resource not found")
	if errors.Is(err1, err2) {
...
func TestUnwrapGRPC_DifferentErrors(t *testing.T) {
	...
	tests := []struct {
		name         string
		grpcError    error
		expectedCode ErrorConstants
		expectedMsg  string
	}{
		{
			name:         "NotFound with details",
			grpcError:    createGRPCError(ErrorConstants_NOT_FOUND, "not found detail"),
			expectedCode: ErrorConstants_NOT_FOUND,
			expectedMsg:  "not found detail",
		},
		{
			name:         "Invalid tx with details",
			grpcError:    createGRPCError(ErrorConstants_INVALID_TX_DOUBLE_SPEND, "double spend detail"),
			expectedCode: ErrorConstants_INVALID_TX_DOUBLE_SPEND,
			expectedMsg:  "double spend detail",
		},
		{
			name:         "Invalid block with details",
			grpcError:    createGRPCError(ErrorConstants_INVALID_BLOCK, "invalid block detail"),
			expectedCode: ErrorConstants_INVALID_BLOCK,
...
func Test_UnwrapChain(t *testing.T) {
	baseErr := New(ErrorConstants_INVALID_TX_DOUBLE_SPEND, "base error")
...
func Test_GRPCErrorsRoundTrip(t *testing.T) {
	originalErr := New(ErrorConstants_BLOCK_NOT_FOUND, "not found")
...
func createGRPCError(code ErrorConstants, msg string) error {
