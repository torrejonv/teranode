...
A broadcast service would be able to validate a transaction almost in full if the sender would also send the missing data (previous locking scripts, satoshi outputs, block height, and coinbase indicator) from the UTXOs being used in the transaction. When creating a new transaction, the previous locking scripts, satoshi outputs, and block height are needed to be able to properly sign the transaction, so the missing data is available at the time of the transaction creation. Serializing the transaction to Extended Format, instead of the standard format, is at the point of creating the transaction no extra work, but does make it much easier for a broadcast service to validate the transaction when being received, before sending the transaction to a node.
...
In the Extended Format, we extend the input structure to include the previous locking script, satoshi outputs, block height, and coinbase indicator:
...
| **Block height + coinbase flag** | **Block height (31 bits) + coinbase flag (1 bit)**                                        | **4 bytes**                     |

The block height and coinbase flag are stored together using bit shifting, where 31 bits represent the block height and the remaining bit indicates whether the input UTXO originates from a coinbase transaction.

## Backward compatibility

The Extended Format is not backwards compatible but has been designed in such a way that existing software should not read a transaction in Extended Format as a valid (partial) transaction. The Extended Format header (`0000000000EF`) will be read as an empty transaction with a future `nLocktime` in a library that does not support the Extended Format.
