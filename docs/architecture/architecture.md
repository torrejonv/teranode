...
4. **Coinbase Overlay Node**: This service handles the Coinbase transactions, which are the first transactions in a block that create new coins and reward miners. ****HELP - Creates them??? what does this do? ** END HELP**
...
5. **UBSV Node Core Services**:
  ...
  - **Public Endpoints Service**: Provides API endpoints for external entities to interact with the node. **HELP**

6. **Ancillary Services**:
  - **TX Submission (ARC)**: Manages the submission of new transactions to the network. **HELP**
  - **Banlist Service**: Maintains and checks a list of banned entities or nodes. **HELP**
  - **UTXO Lookup Service**:
...
7. **Other Components**:
  ...
  - **TX Status Store**: Keeps track of the status of transactions within the system.  **HELP**
...
The UBSV data model addresses scalability and efficiency issues found in traditional Bitcoin by changing the way data is organized and propagated across the network. Here's a summary of the key points and how they represent improvements over the historical Bitcoin model:
...
We can compare the data models of traditional Bitcoin and UBSV to understand how the latter improves on the former.

#### Bitcoin

In traditional Bitcoin the data model is as follows:
...
* ##### Transactions:
  - **UBSV**: Transactions include additional metadata to facilitate processing, and are broadcast to nodes as they occur.

* ##### SubTrees:
  - **Unique to UBSV**: Acts as an intermediate data structure to hold batches of 1M transaction IDs (including metadata) and their corresponding Merkle root.
...
5. **Block Assembly**: The Block Assembly Service compiles block templates consisting of subtrees and their transactions. **HELP - is this true?** These templates are pre-blocks that the node miner service will use to create a full block. Once a hashing solution has been found, the block is broadcasted to all other nodes for validation.


6. **Block Validation**: Once a node finds a valid hash (a successful proof-of-work), the found block is sent to the Block Validation Service. This service checks the new block against the network's consensus rules. If the block is valid, the node will append it to their version of the blockchain.
...
--- NOTE - WHERE IS THE CODE FOR THE TRANSACTION STORE? IS IT A SERVICE?? **HELP**

- Technology and specific Stores [TODO] **HELP**
  - TX Storage...

**HELP**  - in this diagram, we have an extended transaction, but it is not clear what the extended transaction is. Is it the transaction with the metadata? Why does it come from the Multicast Group? Who originally assembles the "Extended TX"???? **HELP**
...
** HELP - What is the TX METADATA Store for? Include here in the details? ** HELP
...
The Validator service is responsible for validating subtrees, and blocks. It is the core of the node, and it is the only service that interacts with the blockchain. The Validator service is also responsible for maintaining the UTXO set.


The UBSV Block Validation Service listens to announcements of blocks found by other miners, and propagates a block this node has found to the network.
Blocks are propagated as the top part of the Merkle tree of the subtrees of the block, so a small list of subtree hashes. All subtrees should have been announced by other miners, validated by the current node, and be available for the node to quickly validate the found block.
If we have not "blessed" a transaction that is being sent to us in a subtree, we need to request that transaction from the miner, validate it and add it to our block assembly. If the transaction cannot be validated, because of missing inputs or it is a double spend, the subtree cannot be "blessed" and that transaction needs to be marked for further processing if the miner wins the block.
All subtrees announced by other nodes are received and stored in the Merkle subtree store, that stores the subtrees until a new block has been found and the subtrees are no longer needed.
The announced subtrees are immediately validated (missing transactions etc.), for later processing in case a block is found by a competing miner.
...
Given the context provided, let's re-examine the components of the diagram and the process of block validation:
...
2. **TX Storage Service:** Maintains a record of all transactions that have not yet been confirmed and added to a block on the blockchain.
...
4. **Block Validation Subtree Micro-Service:** These services are likely tasked with validating the individual subtrees of a block. They operate in parallel to handle the validation workload efficiently, as indicated by multiple instances (N-N-).
...
6. **TX Point to Point Request Network (IPv6):** This is utilized to request the full transaction data for transactions that are not "blessed." The use of IPv6 indicates a robust and scalable network.
...
From the context given, the "Block Validation" text outlines a procedural approach:

- **Listening and Propagation:** The UBSV Block Validation Service listens for new blocks announced by other miners and propagates its own blocks to the network. It only propagates the top part of the Merkle tree, which consists of the hashes of the subtrees, ensuring efficiency in network transmission.

- **Blessing Transactions:** When a transaction is included in a subtree but has not been pre-validated ("blessed") by this node, it requires validation. If validation fails due to issues like missing inputs or double spending, the subtree isn't blessed, and the transaction is set aside for extra scrutiny if the block wins.

- **Storing and Validating Subtrees:** Received subtrees are stored in the Merkle subtree store and are immediately validated to check for any issues such as missing transactions. This step ensures that the node is prepared in case a block is found by another miner.

This design implies a system where nodes actively participate in the network, ensuring the integrity of the blockchain by validating transactions and blocks efficiently. The process is designed to handle high loads and verify transactions promptly to maintain the network's security and trustworthiness.
...
***   **HELP** - What is the Seeder Service? **HELP**   ***

The Seeder service is responsible for seeding the network with new nodes. It is also responsible for receiving new nodes from other nodes and forwarding them to the Validator service.
...
*** HELP - What is the Blob Server Service? Tie up with the model *** HELP
...
*** HELP - What is the Bootstrap Service? Tie up with the model *** HELP
...
1. **P2P Network (<IPv4>)**: This refers to the traditional peer-to-peer network using the IPv4 internet protocol.
