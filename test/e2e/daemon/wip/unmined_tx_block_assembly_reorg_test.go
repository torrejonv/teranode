...
func testUnminedTransactionInBlockAssemblyAfterReorg(t *testing.T, utxoStore string) {
	...
	err = td.BlockValidationClient.ProcessBlock(td.Ctx, block3A, block3A.Height)
	...
	err = td.BlockValidationClient.ProcessBlock(td.Ctx, block3B, block3B.Height)
	...
	err = td.BlockValidationClient.ProcessBlock(td.Ctx, block4B, block4B.Height)
