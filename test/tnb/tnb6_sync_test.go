...
type TNB6SyncTestSuite struct {
	helper.TeranodeTestSuite
}

func TestTNB6SyncTestSuite(t *testing.T) {
	suite.Run(t, &TNB6SyncTestSuite{
...
// $ cd test/tnb/
// $ go test -v -run "^TestTNB6TestSuite$/TestUTXOSetManagement$" -tags test_tnb
func (suite *TNB6SyncTestSuite) TestUTXOSetManagement() {
	synctest.Run(func() {
		...
		// Create a transaction with input coming from the faucet
		tx := bt.NewTx()
		...
		// Add 2 outputs with different amounts
		...
		// Replace time.Sleep, waiting that the first output appears into the UTXO store.
		// This logic checks periodically if the UTXO is present.
		utxoUpdated := false
		go func() {
			for {
				// Let's suppose that UtxoStore.Get returns (utxo, nil) if UTXO esists
				if utxo, err := node1.UtxoStore.Get(ctx, faucetTx.TxIDChainHash()); err == nil && utxo != nil {
					// Once UTXO is found, utxoUpdated = true
		...
		// Wait for AfterFunc being executed
		synctest.Wait()
		require.True(t, utxoUpdated, "UTXO update did not occur in time")
		require.NoError(t, err, "First output should be in UTXO set")

		// Now, let's try to spend a UTXO with an invalid transaction
		...
		require.Error(t, err, "Spending with invalid signature should fail")
	})
}
