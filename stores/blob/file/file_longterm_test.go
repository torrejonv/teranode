...
func TestFileLongtermStorage(t *testing.T) {
	...
	t.Run("file retrieval from persistent storage", func(t *testing.T) {
		...
		err = f.Set(context.Background(), testKey, testValue)
		require.NoError(t, err)

		// File should exist
		exists, err := f.Exists(context.Background(), testKey)
		require.NoError(t, err)
		require.True(t, exists)

		// Get should succeed
		value, err := f.Get(context.Background(), testKey)
		require.NoError(t, err)
		require.Equal(t, testValue, value)

		// Get the filenames for verification
		primaryFilename, err := f.options.ConstructFilename(f.path, testKey)
		require.NoError(t, err)

		// Manually copy the file to the persistent directory to simulate what longterm storage does
		persistFilename, err := f.options.ConstructFilename(filepath.Join(f.path, persistSubDir), testKey)
		require.NoError(t, err)
		...
		reader, err := f.GetIoReader(context.Background(), testKey)
		require.NoError(t, err)
...
func TestFileGetDAH(t *testing.T) {
	...
	t.Run("non-existent file should return error", func(t *testing.T) {
		...
		_, err := store.GetDAH(context.Background(), nonExistentKey)
		require.Error(t, err)
	...
	t.Run("file with DAH should return correct value", func(t *testing.T) {
		...
		// Verify DAH value
		dah, err := store.GetDAH(context.Background(), testKey)
		require.NoError(t, err)
		require.Equal(t, defaultDAH, dah)

		// Set custom DAH
		customDAH := uint32(200)
		err = store.SetDAH(context.Background(), testKey, customDAH)
		require.NoError(t, err)
