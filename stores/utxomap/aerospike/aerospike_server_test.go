...
func internalTest(t *testing.T) {
	...
	aeroURL, err := url.Parse(fmt.Sprintf("aerospike://%s:%d/%s?set=%s", aerospikeHost, aerospikePort, aerospikeNamespace, aerospikeSet))
	...
	t.Run("aerospike get", func(t *testing.T) {
		...
		resp, err := db.GetSpend(context.Background(), spend.TxID)
		require.NoError(t, err)
	...
	t.Run("aerospike store", func(t *testing.T) {
		...
		txMeta, err = db.Create(context.Background(), tx)
		assert.Nil(t, txMeta)
		require.ErrorIs(t, err, txmeta.NewErrTxmetaAlreadyExists(tx.TxIDChainHash()))
