...
func TestSQL_GetBlockHeaders(t *testing.T) {
	...
	t.Run("", func(t *testing.T) {
		...
		assert.Equal(t, uint32(2), heights[0])
		assert.Equal(t, block1.Header.Hash(), headers[1].Hash())
		assert.Equal(t, uint32(1), heights[1])

		headers, heights, err = s.GetBlockHeaders(context.Background(), block1.Hash(), 2)
		require.NoError(t, err)
		assert.Equal(t, 2, len(headers))
		assert.Equal(t, block1.Header.Hash(), headers[0].Hash())
		assert.Equal(t, uint32(1), heights[0])
		assert.Equal(t, "000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f", headers[1].Hash().String())

		headers, heights, err = s.GetBlockHeaders(context.Background(), block1.Hash(), 1)
		require.NoError(t, err)
		assert.Equal(t, 1, len(headers))
		assert.Equal(t, block1.Header.Hash(), headers[0].Hash())
		assert.Equal(t, uint32(1), heights[0])

		headers, heights, err = s.GetBlockHeaders(context.Background(), block1.Hash(), 10)
		require.NoError(t, err)
		...
		headers, heights, err = s.GetBlockHeaders(context.Background(), block2_alt.Hash(), 10)
		require.NoError(t, err)
