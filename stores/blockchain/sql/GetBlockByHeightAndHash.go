...
func (s *SQL) GetBlockByHeightAndHash(ctx context.Context, height uint32, startHash *chainhash.Hash) (*model.Block, error) {
	ctx, _, deferFn := tracing.StartTracing(ctx, "sql:GetBlockByHeightAndHash")
	defer deferFn()

	// the cache will be invalidated by the StoreBlock function when a new block is added, or after cacheTTL seconds
	cacheID := chainhash.HashH([]byte(fmt.Sprintf("GetBlockByHeightAndHash-%d-%s", height, startHash.String())))
