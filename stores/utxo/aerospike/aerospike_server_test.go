...
func TestAerospike(t *testing.T) {
	...
	coinbaseKey, err = aerospike.NewKey(db.namespace, db.setName, coinbaseTx.TxIDChainHash()[:])
	require.NoError(t, err)
	...
	key, err = aerospike.NewKey(db.namespace, db.setName, spendingTxID1[:])
	require.NoError(t, err)

	txKey, err := aerospike.NewKey(db.namespace, db.setName, tx.TxIDChainHash().CloneBytes())
	require.NoError(t, err)

	opReturnKey, aErr := aerospike.NewKey(db.namespace, db.setName, txWithOPReturn.TxIDChainHash().CloneBytes())
...
func TestCoinbase(t *testing.T) {
	...
	key, aErr := aerospike.NewKey(db.namespace, db.setName, coinbaseTx.TxIDChainHash().CloneBytes())
	require.NoError(t, aErr)
	assert.NotNil(t, key)
...
//	require.NoError(t, err)
//
//	key, err := aerospike.NewKey(db.namespace, db.setName, bigTx.TxIDChainHash().CloneBytes())
//	require.NoError(t, err)
//	assert.NotNil(t, key)
//
...
//	require.NoError(t, err)
//
//	key0, err := aerospike.NewKey(db.namespace, db.setName, bigTx.TxIDChainHash().CloneBytes())
//	require.NoError(t, err)
...
//	assert.Len(t, utxos, 2)
//
//	key2, err := aerospike.NewKey(db.namespace, db.setName, uaerospike.CalculateKeySource(bigTx.TxIDChainHash(), 2))
//	require.NoError(t, err)
...
func TestStoreDecorate(t *testing.T) {
	...
	key, aErr := aerospike.NewKey(db.namespace, db.setName, spendingTxID1[:])
	require.NoError(t, aErr)
...
//
//	keyParent, err := aerospike.NewKey(db.namespace, db.setName, parentTx.TxIDChainHash().CloneBytes())
//	require.NoError(t, err)
...
//
//	keyChild, err := aerospike.NewKey(db.namespace, db.setName, childTx.TxIDChainHash().CloneBytes())
//	require.NoError(t, err)
...
func initAerospike(t *testing.T) (*aerospike.Client, *Store, context.Context, func()) {
