...
func main() {
	...
	if !ok {
		panic(errors.New("error getting p2p_dht_protocol_id"))
	}

	sharedKey, ok := gocore.Config().Get("p2p_shared_key")
	if !ok {
		panic(errors.New("error getting p2p_shared_key"))
	}

	privkeyHex, ok := gocore.Config().Get("p2p_bootstrap_privkey")
	if !ok {
		panic(errors.New("error getting p2p_bootstrap_privkey"))
	}

	listenAddr, ok := gocore.Config().Get("p2p_bootstrap_listenAddress")
	if !ok {
		panic(errors.New("error getting p2p_bootstrap_listenAddress"))
	}

	listenPort, ok := gocore.Config().GetInt("p2p_bootstrap_listenPort")
	if !ok {
		panic(errors.New("failed to get p2p_bootstrap_listenPort"))
	}
