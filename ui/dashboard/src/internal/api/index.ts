...
export function getBlocks(data: { offset: number; limit: number }): Promise<ApiResponse<any>> {
  return get<any>(`${baseUrl}/blocks`, { query: { offset: String(data.offset), limit: String(data.limit) } })
}
...
export function getSubtreeTxs(data: { hash: string; offset: number; limit: number }): Promise<ApiResponse<any>> {
  return get<any>(
