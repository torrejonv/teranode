...
export const nodes: any = writable([])
export const sock: Writable<any> = writable(null)

let cancelFunction: any = null
...
export const selectedNode = writable('', (set: any) => {
...
export function connectToBootstrap(assetHTTPAddress: string) {
  ...
  socket.onmessage = async (event) => {
    try {
      ...
      if (json.type === 'ADD') {
        ...
        nodesData = nodesData.sort((a: any, b: any) => {
          if (a.name < b.name) {
            return -1
...
export async function getNodeHeader(address: string) {
  if (address) {
    try {
      const header = await Promise.race([getBestBlockHeader(address), timeout(1000)])
...
async function getBestBlockHeader(address: string) {
